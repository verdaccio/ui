...
const AutoComplete = <Option extends {}>({
  ...
  const filterOptions = useCallback(() => {
    const filteredOptions = defaultFilterOptions(suggestions, { inputValue: searchTerm, getOptionLabel });
  ...
  const renderOptions = useCallback(() => {
    if (renderOptionProp) {
      return options.map((option, index) => (
        <Option isSelected={index === activeOption} key={index} onClick={handleClickOption(option)} tabIndex={0}>
          ...
          endAdornment: (
            <EndAdornment>
              {hasClearIcon && searchTerm.length > 0 && (
                <IconButton color="inherit" onClick={handleClear} size="small" title={t('autoComplete.clear')}>
