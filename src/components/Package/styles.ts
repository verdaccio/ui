...
export const OverviewItem = styled('span')<{ theme: Theme }>(props => ({
  ...
  alignItems: 'center',
  margin: '0 0 0 16px',
  color: props.theme.palette.greyLight2,
  fontSize: 12,
...
export const Icon = styled(Ico)<{ theme: Theme }>(props => ({
  margin: '2px 10px 0 0',
  fill: props.theme.palette.greyLight2,
}));
...
export const PackageTitle = styled('span')<{ theme: Theme }>(props => ({
  ...
  display: 'block',
  marginBottom: 12,
  color: props.theme.palette.eclipse,
  cursor: 'pointer',
  ':hover': {
...
export const PackageList = styled(List)<{ theme: Theme }>(props => ({
  ...
  ':hover': {
    backgroundColor: props.theme.palette.greyLight3,
  },
  '> :last-child': {
    paddingTop: 0,
  },
